...
def get_ad_all(goods_id: int = 221058511472, days: int = 7) -> AjaxResult:
    columns = ['click', 'impression', 'orderNum', 'gmv', 'spend', '_ctr', '_cvr', '_roi']
    
    def handle(name):
        df = pd.DataFrame(calc_ad_item(name, goods_id, days), columns=['date'] + columns)
        df.set_index('date', inplace=True)
        df.rename(columns=dict((i, i + '_' + name) for i in columns), inplace=True)
        return df
