...
from settings import COLL_ORDERS, DEFAULT_GOODS_ID, SECONDS_EACH_DAY, EXCLUDE_ORDER_STATUS_LIST
...
def analyze_orders_of_goods(
    ...
    def get_pivot(key: str) -> pd.DataFrame:
        df = pd.DataFrame(items, columns=['goods_number', 'goods_amount', 'spec', 'date']) \
            .pivot_table(values=key, index='date', columns='spec') \
